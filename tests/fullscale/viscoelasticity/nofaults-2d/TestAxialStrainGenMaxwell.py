...
class TestCase(FullTestCase):
    """Test suite for testing generalized Maxwell material with 2-D axial extension (Dirichlet BC).
    """
    DIRICHLET_BOUNDARIES = ["bc_xneg", "bc_xpos", "bc_ypos", "bc_yneg"]
...
class TestQuad(TestCase, meshes.Quad):
    NAME = "axialstrain_genmaxwell_quad"

    def setUp(self):
        TestCase.setUp(self)
        TestCase.run_pylith(self, self.NAME, [
                            "axialstrain_genmaxwell.cfg", "axialstrain_genmaxwell_quad.cfg"])
        return


# ----------------------------------------------------------------------------------------------------------------------
class TestTri(TestCase, meshes.Tri):
    NAME = "axialstrain_genmaxwell_tri"

    def setUp(self):
        TestCase.setUp(self)
        TestCase.run_pylith(self, self.NAME, [
                            "axialstrain_genmaxwell.cfg", "axialstrain_genmaxwell_tri.cfg"])
        return
